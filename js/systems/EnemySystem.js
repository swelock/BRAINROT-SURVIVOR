import {endlessPressure} from './ModeSystem.js';
...
export function spawnEnemy(g,type,boss=false){
 ...
 const pressure=g.mode==='ENDLESS'?endlessPressure(g.endlessTime):null;
 const elite=!boss&&g.wave>=8&&g.rng()<(pressure?.elite??(.06+(g.wave-8)*.008));
 ...
 if(pressure){e.hp*=pressure.hp;e.maxHp=e.hp;e.damage*=pressure.damage;e.attackPower=pressure.damage;e.speed*=pressure.speed;}
 if(g.mode==='SUPER_VOMIT'&&boss){const round=g.superRound;e.hp*=.58+round*.17;e.maxHp=e.hp;e.damage*=.75+round*.055;e.attackPower=.85+round*.07;e.speed*=1+Math.min(.28,round*.022);e.patternRate=Math.min(1.65,1+round*.045);}
 e.hp=Math.round(e.hp);e.maxHp=e.hp;g.enemies.push(e);if(boss){g.boss=e;g.sound('boss');g.effect(e.x,e.y,160,'#ed9e91',1.5);g.announce('boss',type);}return e;
}
...
export function updateEnemies(g,dt){const p=g.player;
 for(const e of g.enemies){if(!g.active())return;if(e.dead)continue;e.hit=Math.max(0,e.hit-dt);e.orbitHit=Math.max(0,e.orbitHit-dt);e.freeze=Math.max(0,e.freeze-dt);
 ...
 e.attackTimer-=dt*(e.patternRate||1);
 if(e.boss){if(g.mode==='SUPER_VOMIT'&&d>450)move=Math.max(move,p.character.speed*(1+(p.b.speed||0))*1.18);e.phase=e.type==='absolute'?(e.hp/e.maxHp<.33?3:e.hp/e.maxHp<.66?2:1):1;
...
function executeBoss(g,e,w){const phase=e.phase,power=e.attackPower||1;
 if(w.kind==='charge'){e.charge=.8;e.chargeAngle=w.angle;}
 if(w.kind==='slam'||w.kind==='swing'){const x=w.kind==='swing'?e.x:w.x,y=w.kind==='swing'?e.y:w.y;g.danger(x,y,w.r,(22+phase*8)*power);}
 if(w.kind==='burst'){for(let i=0;i<12+phase*4;i++)enemyBullet(g,e.x,e.y,i*Math.PI*2/(12+phase*4),165+phase*18,15*power,7,e);g.effect(e.x,e.y,100,'#edb17e',.5);}
 if(w.kind==='rain'){for(let i=0;i<8+phase*4;i++){const x=w.x+(g.rng()-.5)*600,y=w.y+(g.rng()-.5)*380;g.zones.push({x,y,r:48,life:1.3,total:1.3,hostile:true,damage:24*power,warning:true});}}
 if(w.kind==='summon'){for(let i=0;i<4+phase*2&&g.enemies.length<240;i++)spawnEnemy(g,pick(['runner','clubber','weird'],g.rng));g.effect(e.x,e.y,150,'#ae8bd8',.9);}
 if(w.kind==='shift'){g.realityShift=6;for(let i=0;i<16;i++)enemyBullet(g,e.x,e.y,i*Math.PI/8,180,18*power,7,e);g.announce('shift');}
}
