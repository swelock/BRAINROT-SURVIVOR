...
export class AssetManager{
 ...
 draw(c,id,x,y,scale=1,t=0,facing=1,hit=false,pose={}){
  ...
  const attacking=(pose.attackPose||0)>0;const a=attacking?pose.attackAngle:pose.lastX===undefined?(facing<0?Math.PI*.75:Math.PI*.25):Math.atan2(pose.lastY,pose.lastX);
  ...
  c.save();const recoil=attacking?(pose.attackPose/.085)*(id==='tung'?3:id==='shark'?2:id==='croc'?2.5:1.5):0;c.translate(x-Math.cos(a)*recoil,y+22*scale-Math.sin(a)*recoil);if(attacking)c.scale(1+recoil*.015,1-recoil*.015);c.rotate(moving?(id==='croc'?.055:.025)*Math.sin(t*9):0);
