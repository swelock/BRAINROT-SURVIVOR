...
function release(){keys.clear();touch={x:0,y:0};game.input={x:0,y:0,dash:false};document.querySelector('#stick i').style.transform='';game.pause();}
window.addEventListener('blur',release);document.addEventListener('visibilitychange',()=>{if(document.hidden)release();});window.addEventListener('resize',()=>renderer.resize());
const stick=document.querySelector('#stick');let stickPointer=null;
...
function frame(now){const dt=Math.min(.1,(now-last)/1000);last=now;game.input.x=Number(keys.has('KeyD')||keys.has('ArrowRight'))-Number(keys.has('KeyA')||keys.has('ArrowLeft'))+touch.x;game.input.y=Number(keys.has('KeyS')||keys.has('ArrowDown'))-Number(keys.has('KeyW')||keys.has('ArrowUp'))+touch.y;
 if(game.active()){accumulator+=dt;while(accumulator>=1/60){game.update(1/60);accumulator-=1/60;}}else accumulator=0;
