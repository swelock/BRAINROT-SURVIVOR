...
export class Game{
 ...
 this.camera={x:0,y:0};this.grid.rebuild([]);this.input.dash=false;if(mode==='SUPER_VOMIT')prepareSuper(this);else{this.announce('wave',1);this.onEvent('state');}return true;}
 ...
 update(dt){dt=Math.min(dt,.05);if(!this.active())return;const p=this.player;this.time+=dt;p.specialTime=Math.max(0,p.specialTime-dt);this.shake=Math.max(0,this.shake-dt*16);this.realityShift=Math.max(0,this.realityShift-dt);
 ...
 if(e.boss){this.bosses++;this.player.hp=Math.min(this.player.maxHp,this.player.hp+this.player.maxHp*(this.mode==='SUPER_VOMIT'?.08:.2));this.chests.push({x:e.x,y:e.y,r:23});bossDefeated(this,e);}}
 }
