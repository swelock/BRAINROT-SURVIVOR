...
export class Game{
 ...
 update(dt){dt=Math.min(dt,.05);if(!this.active())return;const p=this.player;this.time+=dt;this.shake=Math.max(0,this.shake-dt*16);this.realityShift=Math.max(0,this.realityShift-dt);
 ...
 if(this.state==='WAVE_TRANSITION'){this.transition-=dt;if(this.transition<=0)this.state='PLAYING';}
 ...
 this.grid.rebuild(this.enemies);updateWeapons(this,dt);if(!this.active())return;
 ...
 this.enemies=this.enemies.filter(e=>!e.dead);this.effects=this.effects.filter(e=>(e.life-=dt)>0);this.numbers=this.numbers.filter(n=>{n.y-=25*dt;return(n.life-=dt)>0;});this.particles=this.particles.filter(q=>{q.x+=q.vx*dt;q.y+=q.vy*dt;q.vy+=35*dt;return(q.life-=dt)>0;});
 ...
 damageEnemy(e,base,forceCrit=false,dot=false){if(e.dead)return;const p=this.player,b=p.b;const crit=!dot&&(forceCrit||this.rng()<.05+(b.crit||0));const nearby=b.apocalypse?this.grid.query(p.x,p.y,150).filter(e=>!e.dead&&dist(e,p)<150).length:0;
 ...
 this.totalDamage+=Math.min(e.hp,damage);e.hp-=damage;if(!dot){e.hit=.12;if(this.numbers.length<50)this.numbers.push({x:e.x,y:e.y-e.r,value:Math.round(damage),crit,life:.65});if(crit)this.sound('crit');}
 if(e.hp<=0){e.dead=true;this.kills++;this.sound('death');this.particle(e.x,e.y,e.color,7);const value=e.boss?95:(e.xp||8)*(e.elite?3:1);if(this.pickups.length>=700){let nearest=this.pickups[0];for(const q of this.pickups)if(dist(q,e)<dist(nearest,e))nearest=q;nearest.value+=value;}else this.pickups.push({x:e.x,y:e.y,value});
 if(e.modifier==='explosive')this.zones.push({x:e.x,y:e.y,r:85,life:1,total:1,hostile:true,damage:18});
 if(e.boss){this.bosses++;this.player.hp=Math.min(this.player.maxHp,this.player.hp+this.player.maxHp*.2);this.chests.push({x:e.x,y:e.y,r:23});if(e.type==='absolute')this.finish(true);}}
 }
 explode(x,y,r,damage,color='#d8e886',knockback=55){r*=1+(this.player.b.explosionRadius||0);this.effect(x,y,r,color,.45);this.particle(x,y,color,12);for(const e of this.grid.query(x,y,r+65))if(!e.dead&&Math.hypot(e.x-x,e.y-y)<r+e.r){this.damageEnemy(e,damage*(1+(this.player.b.explosion||0))*(1+(this.player.b.area||0)));const a=Math.atan2(e.y-y,e.x-x);e.x+=Math.cos(a)*knockback;e.y+=Math.sin(a)*knockback;}this.shake=Math.max(this.shake,2);}
 chain(origin,damage){const targets=this.grid.query(origin.x,origin.y,240).filter(e=>!e.dead&&e!==origin).sort((a,b)=>dist(a,origin)-dist(b,origin)).slice(0,3);let from=origin;for(const e of targets){this.effects.push({x:from.x,y:from.y,x2:e.x,y2:e.y,lightning:true,life:.2,total:.2,color:'#aadefa'});this.damageEnemy(e,damage);from=e;}}
 hurt(damage,source){const p=this.player;if(p.invulnerable>0||this.hasBuff('invincible')||this.rng()<(p.b.dodge||0))return;
 ...
 finish(victory){if(this.ended)return;this.ended=true;this.state=victory?'RESULTS':'GAME_OVER';const run={victory,wave:this.wave,time:this.time,kills:this.kills,bosses:this.bosses,level:this.player.level,damage:Math.round(this.totalDamage),bestMultiplier:this.bestMultiplier};this.result={...run,...awardRun(this.save,run,this.player.b,this.rng)};this.sound(victory?'victory':'lose');this.onEvent('save');this.onEvent('end',this.result);}
}
